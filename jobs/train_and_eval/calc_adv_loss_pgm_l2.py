...
from attacks import adversarial_attack
...
def main(config, args):
    ...
    phis_2 = []
    results = {}
    for eps in [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0, 200.0, 500.0, 1000.0]:
        alpha = eps / (1 + np.log(config.steps)) * 0.1
        with chainer.using_config('train', False):
            ...
            for _ in range(0, len(iterator.dataset), iterator.batch_size):
                ...
                x_adv = adversarial_attack(cls=cls, x=x, y=y, steps=config.steps, eps=eps,
                                           loss_type=config.loss_type, clip_x=config.clip_x, alpha=alpha)
                logit = cls(x_adv)
